#!/usr/bin/env node
/**
 * Synthetic PerLTQA 召回率基准测试 - Ollama Qwen3-Embedding-8B 版
 * 使用本地 GPU (AMD RX 6800 ROCm)
 */
...
async function runScaleTest(db, scaleName) {
  ...
  const scope = `perltqa-qwen3-${Date.now()}`;
  
  // 生成记忆 embedding
  console.log(`\n  🚀 生成记忆 embedding (Qwen3-8B GPU)...`);
...
async function main() {
  ...
  console.log('🎮 GPU: AMD RX 6800 ROCm (via Ollama)');
  console.log('⚡ 预期速度：比 8B 快 2 倍');
  console.log('='.repeat(70));
  ...
  const report = {
    ...
    device: 'AMD RX 6800 ROCm (Ollama)',
    results: allResults,
    timings: allTimings
  };
  
  const outputPath = path.join(config.dataDir, 'benchmark_result_qwen3_ollama.json');
